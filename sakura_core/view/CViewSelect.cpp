		...
		if( m_sSelect != m_sSelectOld ){
			...
			if( !m_sSelect.IsValid() ){
				...
				if(m_sSelect.GetFrom() == m_sSelectOld.GetFrom() && m_sSelect.GetTo().x != m_sSelectOld.GetTo().x){
					...
					drawLeft  = t_min(m_sSelect.GetTo().x, m_sSelectOld.GetTo().x) - 2; // 合成文字対策で-2
					drawRight = t_max(m_sSelect.GetTo().x, m_sSelectOld.GetTo().x) + 4; // 合成文字対策で+4
					...
					drawLeft  = t_min(m_sSelectOld.GetFrom().x, m_sSelect.GetFrom().x) - 2; // 合成文字対策で-2
					drawRight = t_max(m_sSelectOld.GetFrom().x, m_sSelect.GetFrom().x) + 4; // 合成文字対策で+4
				}else{
					rc.UnionStrictRect(rcOld, rcNew);
				}
