...
//	変数削除
	...
	if( NULL != m_cStatusBar.GetStatusHwnd() ){
		...
		const TCHAR*	pszLabel[nStArrNum] = { _T("(99999, 9999)"), _T(""), _T("CRLF(Win)"), _T("AAAAAAAAAAAA"), _T("Unicode BOM付"), _T("REC"), _T("上書"), _T("Tab 99[SP]"), _T("123456789012") };
#else
...
LRESULT CEditWnd::PopupWinList( bool bMousePos )
{
	...
	if( bMousePos ){
		...
		pt.x = rc.left + RegKey(REI_REGKEY).get(_T("WinListPopupLeft"), 2);
		pt.x = std::max(std::min(pt.x, rc.right), rc.left);
		pt.y = rc.top + RegKey(REI_REGKEY).get(_T("WinListPopupTop"), 2);
