...
#if defined(NKMM_FIX_TABWND) && NKMM_TAB_CLOSE_BTN_DRAW == 1
	...
	if( lpdis->CtlType == ODT_MENU )
	{
...
#if defined(NKMM_FIX_TABWND) && NKMM_BUGFIX_TAB_EDGE == 1
		...
		if (clrText == ::GetSysColor(COLOR_MENUTEXT)) {
			...
			if (p && p->m_bIsRecMacro) {
				TCHAR szData[32];
				if (RegKey(NKMM_REGKEY).read(_T("TabCaptionRecMacroColor"), (LPCTSTR)szData)) {
					clrText = si::ColorString::ToCOLORREF(szData);
				}
				else {
					clrText = si::ColorString::ToCOLORREF(NKMM_RECMACRO_TAB_CAPTION_COLOR);
				}
			}
		}
		if (clrText == ::GetSysColor(COLOR_MENUTEXT)) {
			...
			if (p && p->m_bIsModified) {
				TCHAR szData[32];
				if (RegKey(NKMM_REGKEY).read(_T("TabCaptionModifiedColor"), (LPCTSTR)szData)) {
					clrText = si::ColorString::ToCOLORREF(szData);
				}
				else {
					clrText = si::ColorString::ToCOLORREF(NKMM_MODIFIED_TAB_CAPTION_COLOR);
				}
			}
		}
#endif // NKMM_
...
HIMAGELIST CTabWnd::InitImageList( void )
{
	...
	hImlNew = NULL;
#ifndef NKMM_FIX_TAB_CAPTION_COLOR
	if( m_pShareData->m_Common.m_sTabBar.m_bDispTabIcon )
#endif // NKMM_
	{
		// システムイメージリストを取得する
