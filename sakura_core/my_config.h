...
// 修正者
#define NK_AUTHOR       "Niki"
#define NK_AUTHOR_PAGE  "https://github.com/niki/sakura"

// 拡張用レジストリキー
#define NK_REGKEY _T("Software\\sakura-niki")

//------------------------------------------------------------------
// バージョン情報ダイアログの変更 20170315
//------------------------------------------------------------------
#define NK_FIX_VERDLG
	#define PR_VER      2,60,0,0
	#define PR_VER_STR "2.60"
	#define BASE_REV    4197

//-------------------------------------------------------------------------
// デバッグ出力 20150324
//-------------------------------------------------------------------------
//#define NK_OUTPUT_DEBUG_STRING

//------------------------------------------------------------------
// スクロール
//------------------------------------------------------------------
#define NK_FIX_SCROLL
	// 水平スクロールの変更
	//  - スクロール開始マージンを 1 に変更。画面の端でスクロール開始 20140507
	//  - スクロール幅を 16 に設定。一度に大きく移動することで見やすくする (Imitate 'Notepad') 20150902
	#define NK_HORIZONTAL_SCR (16)
	// 垂直スクロールの変更
	//  - スクロールマージンを 0 に変更 20170409
	#define NK_VERTICAL_SCR (0)
...
//------------------------------------------------------------------
#define NK_FIX_CENTERING_CURSOR_JUMP

//------------------------------------------------------------------
// タブ入力文字の切り替え(タブ<->空白)を追加
//  - S_ChangeTabWidthマクロ修正, 負の値を渡すとタブ文字の切り替え
//------------------------------------------------------------------
#define NK_FIX_CHANGE_TAB_WIDTH_MACRO
...
//------------------------------------------------------------------
#define NK_FIX_SETMAINFONT

//------------------------------------------------------------------
// 'BOM付' を 'BOM' に変更 20170417
//  - わざわざ「付」をつけなくても認識できると判断とスペースの節約
//------------------------------------------------------------------
#define NK_FIX_WITH_BOM_TEXT

//------------------------------------------------------------------
// タブ名カラー
//------------------------------------------------------------------
#define NK_FIX_TAB_CAPTION_COLOR
	// 変更ドキュメントタブ名カラー (REG/TabCaptionModifiedColor:#0000d7)
	#define NK_MODIFIED_TAB_CAPTION_COLOR _T("#0000d7")
	// マクロ記録中ドキュメントタブ名カラー (REG/TabCaptionRecMacroColor:#d80000)
	#define NK_RECMACRO_TAB_CAPTION_COLOR _T("#d80000")
...
#define NK_FIX_EDITVIEW_SCRBAR
	...
	#define NK_SCRBAR_LINEN_MASK  (0x0fffffff)  // 行番号マスク
	#define NK_SCRBAR_MAGIC_MASK  (0xf0000000)  // マジックマスク
	// 検索文字列のある行の色 (REG/EditViewScrBarFoundColor:#0000d7)
	#define NK_SCRBAR_FOUND_COLOR  _T("#f4a721") //_T("#32CD32") //_T("#0000d7")
	// ブックマークのある行の色 (REG/EditViewScrBarMarkColor:#d80000)
	#define NK_SCRBAR_MARK_COLOR   _T("#ff0032") //_T("#ff0000") //_T("#d80000")
	// キャレットのある行の色 (REG/EditViewScrBarMarkColor:#d80000)
	#define NK_SCRBAR_CURSOR_COLOR _T("#0026ff") //_T("#0000cd") //_T("#00d800")
	
	#define NK_EDITVIEW_H_SCRBAR_REDRAW_TIMING  (1)  // 水平スクロールバーの更新タイミングを修正
...
//------------------------------------------------------------------
#define NK_FIX_CARET_WIDTH

//------------------------------------------------------------------
// カーソル行の行番号背景を「カーソル行の背景色」で描画する 20171002 
//------------------------------------------------------------------
#define NK_FIX_CUR_BACK_DRAW

//------------------------------------------------------------------
// EOFのみの行にも行番号を表示 20170310
//------------------------------------------------------------------
#define NK_FIX_EOFLN_DISP_NR
...
//------------------------------------------------------------------
#define NK_FIX_TAB_MARK

//------------------------------------------------------------------
// カーソル行アンダーライン
//  - 行番号を含む左端から 20150130
//------------------------------------------------------------------
#define NK_FIX_CUR_UL
...
//  - カラー設定は背景カラーのみ
//------------------------------------------------------------------
#define NK_FIX_SELAREA
	#define NK_SELAREA_TEXT_BLEND_PER (0)    // 選択領域のブレンド率[%] (REG/SelectAreaTextBlendPer:0x00000000) 20150605
	#define NK_SELAREA_BACK_BLEND_PER (100)  // 選択範囲のブレンド率[%] (REG/SelectAreaBackBlendPer:0x00000064)
	#define NK_SELAREA_BACK_BLEND_PER2 (60)  // 特定の下地のときの選択範囲のブレンド率 (REG/SelectAreaBackBlendPer2:0x0000003C)
...
//------------------------------------------------------------------
#define NK_FIX_WS_COLOR
	#define NK_WS_BLEND_PER (30)  // 空白,TAB,改行,EOF,ノート線 現在のカラーのブレンド率[%] (REG/WhiteSpaceBlendPer:0x0000001E) 20150605
//						     
...
//  - 折り返し記号 テキストカラーのみ
//------------------------------------------------------------------
#define NK_FIX_COLOR_STRATEGY

//------------------------------------------------------------------
// コメント行 20161227
// 0x01: 改行以降もカラーを有効にする (Imitate 'Sublime Text')
//------------------------------------------------------------------
#define NK_FIX_COMMENT

//------------------------------------------------------------------
// 数値の色付け判定
//  - 正規表現で判定する 20170421
//------------------------------------------------------------------
#define NK_FIX_NUMERIC_COLOR
...
//------------------------------------------------------------------
#define NK_FIX_STATUSBAR

//------------------------------------------------------------------
// 折り返しモード 20170603
//  - トグルで切り替えたときに「折り返さない」が処理されていないので修正
//------------------------------------------------------------------
#define NK_FIX_WRAP_MODE
...
//------------------------------------------------------------------
#define NK_BREGONIG_NAME_SEARCH
...
//------------------------------------------------------------------
#define NK_FIX_PROFILES
	#define NK_SEPARATE_HISTORY                    (1)  // 履歴は別ファイルで扱う (sakura.recent) 20170502
	#define NK_DELETE_HISTORY_NOT_EXIST_AT_STARTUP (1)  // 起動時に存在しないファイル・フォルダの履歴は削除する 20170410
	#define NK_USE_KEYWORDSET_CSV                  (1)  // sakura.keywordset.csvを用意し、強調キーワードの管理はこのファイルで行う 20170513

//------------------------------------------------------------------
// メインメニューはデフォルトを使用する 20170515
//  - 共通設定から「メインメニュー」タブを削除します
//------------------------------------------------------------------
#define NK_FIX_MAINMENU_FORCE_DEFAULT
...
//------------------------------------------------------------------
#define NK_MULTIPLE_OPEN_FILES

//------------------------------------------------------------------
// 最大数 20131002, 20161213, 20170618 
// \sakura_core\config\maxdata.h
//------------------------------------------------------------------
#define NK_FIX_MAXDATA
	#define NK_MAX_SEARCHKEY  (20) // 検索キー (REG/RecentSearchKeyMax:20)
	#define NK_MAX_REPLACEKEY (20) // 置換キー (REG/RecentReplaceKeyMax:20)
	#define NK_MAX_GREPFILE   (10)  // Grepファイル (REG/RecentGrepFileMax:10)
	#define NK_MAX_GREPFOLDER (20) // Grepフォルダ (REG/RecentGrepFolderMax:20)
...
//------------------------------------------------------------------
#define NK_FIX_TYPELIST_ADD_ANY_TYPE

//------------------------------------------------------------------
// デフォルト文字コードを UTF8にする 20170622 
//------------------------------------------------------------------
#define NK_FIX_DEFAULT_CHARCODE_TO_UTF8

//------------------------------------------------------------------
// 正規表現検索の際、検索文字列の正規表現記号をクォートする 20150601
// PHPの preg_quote みたいなもの
//------------------------------------------------------------------
#define NK_FIX_SEARCH_KEY_REGEXP_AUTO_QUOTE
...
//  - 中断
//------------------------------------------------------------------
#define NK_FIX_DIALOG_POS

//------------------------------------------------------------------
// タグジャンプ一覧
//  - 表示するカラムの選別と並び替え 20150827
//------------------------------------------------------------------
#define NK_FIX_TAGJUMP

//------------------------------------------------------------------
// ダイアログ
//------------------------------------------------------------------
#define NK_FIX_DLG
	#define NK_COMBO_DROP_ALT_AND_UPDOWN_ONLY      (1)  // コンボボックスのドロップダウンは上下キーでは行えないようにする(誤操作防止) 20170704 
	#define NK_CLOSE_DIALOG_WITH_MODE_CANCELLATION (1)  // モード取り消し時にダイアログもいっしょに閉じる 20170809 

//------------------------------------------------------------------
// タグファイル作成ダイアログ
//  - タグ作成フォルダの初期値を tags ファイルがあるところまで辿る 20170512
//------------------------------------------------------------------
#define NK_FIX_TAGMAKEDLG
...
//------------------------------------------------------------------
#define NK_FIX_FINDDLG

//------------------------------------------------------------------
// アウトライン解析
//  - 正規表現を使用した場合はマッチした文字列のみリストに登録 20170608
//------------------------------------------------------------------
#define NK_FIX_OUTLINE
...
//------------------------------------------------------------------
#define NK_FIX_SEARCH_KEEP_REGEXP

//------------------------------------------------------------------
// 変更行は縦線で描画する 20170731 
//  - 副作用で行番号が非表示のときも状態がわかるようになります
//------------------------------------------------------------------
#define NK_FIX_MODGYOU_DRAW_VLINE

//------------------------------------------------------------------
// ブックマークは縦線で描画する 20170727 
//  - 副作用で行番号が非表示のときも状態がわかるようになります
//------------------------------------------------------------------
#define NK_FIX_BOOKMARK_DRAW_VLINE

//------------------------------------------------------------------
// 偶数行背景はEOF以降は適用しない 20170620 
//------------------------------------------------------------------
#define NK_FIX_NOT_EVEN_LINE_FROM_EOF

//------------------------------------------------------------------
// ノート線はEOF以降は適用しない 20170620 
//------------------------------------------------------------------
#define NK_FIX_NOT_NOTE_LINE_FROM_EOF

//------------------------------------------------------------------
// ::ExtTextOut による塗りつぶしを ::PatBlt に変更 20170708  
//------------------------------------------------------------------
#define NK_FIX_EXTTEXTOUT_TO_PATBLT

//------------------------------------------------------------------
// WM_ERASEBKGNDの抑制 20170708 
//------------------------------------------------------------------
#define NK_FIX_SUPPRESSION_OF_WM_ERASEBKGND

//------------------------------------------------------------------
// タイプ設定のデフォルトを直接設定するようにする 20170714  
//  - なぜstaticで持ってコピーしているのか不明
//------------------------------------------------------------------
#define NK_FIX_TYPE_CONFIG_DEFAULT
