...
//------------------------------------------------------------------
// タイプ別設定ダイアログ(プロパティシート)の左下に「初期化」ボタンを追加し、
// 現在編集中のタイプの設定を、選択した任意のタイプの初期値で置き換えられる
// ようにする 20260913
// NKMM_FIX_TYPELIST_ADD_ANY_TYPEのCreateTypeConfig()/GetTypeNames()を使うため、
// あちらが有効であることが前提
//------------------------------------------------------------------
#define NKMM_FIX_TYPELIST_INIT_ANY_TYPE
...
//------------------------------------------------------------------
// コードフォールディング(文書全体をアウトライン表示⇔全展開でトグル) 20260911
//  - Ctrl+Shift+[ / Ctrl+Shift+](どちらも同じトグル、F_OUTLINE_FOLD_TOGGLE)で、
//    文書全体を関数/構造体等のヘッダ行だけのアウトライン表示にする。
//    ヘッダ行以外(#include・コメント・空行等も含む)は全て非表示になる
//  - 対応するアウトライン種別はC/C++に限らず、組み込みの解析方式全般
//    (Java/Perl/Python/VB/PL-SQL/COBOL/アセンブラ/階層付きテキスト/
//    HTML/XML/TeX/プレーンテキスト)。ブックマーク一覧/ファイルツリー/
//    ルールファイル種別/プラグイン提供種別は対象外
//  - アウトライン表示中は編集操作を禁止し、キャレットは元いたスコープの
//    ヘッダ行へ自動移動する。Enter/Tab/ダブルクリックでその位置のまま
//    全展開(ジャンプ)、Escで折りたたみに入る前のキャレット位置へ戻る
//    (全展開)。ジャンプ先は画面中央に来るようスクロールする
//  - ヘッダ行の関数/メソッド名部分(CFuncInfoの名前をヘッダ行テキストから
//    部分文字列検索して特定。正規表現不使用)を検索マーク色でハイライトする
//  - 折りたたみ範囲はCFuncInfo(アウトライン解析データ)に終了行が無いため、
//    「次に現れる同じか浅い深さの項目の直前まで」を終了行とみなして算出する
//    (CFoldRangeCalculator::Calculate())
//  - 行番号マージンへの+/-アイコン表示・マウスクリックでの個別折りたたみは
//    未実装(前例の無いUIのため、キーバインドのみで操作する方式に絞った)
//  - 詳細はchangelog/NKMM_CODE_FOLDING.md参照(見つけて修正した実クラッシュ
//    (COMCTL32内ヒープ破壊、_TextWidthMultisetErase呼び忘れ、スクロール
//    バーマーカーのキャッシュサイズ取り違え等)の詳細もこちらに記載)
//------------------------------------------------------------------
#define NKMM_CODE_FOLDING
