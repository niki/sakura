...
//------------------------------------------------------------------
// 共通タブバー(社内呼称:方式A) Step 0: タブウィンドウ単体生成の検証 20260815
//    各エディタプロセスが個別に持つCTabWndの代わりに、コントロールプロセス側に
//    1つだけ浮遊ウィンドウ(CUnifiedTabWnd)を作り、全プロセスのタブを集約表示する
//    構想の第一歩。Step 0では「コントロールプロセスがタブコントロールを持つ
//    浮遊ウィンドウを単体で生成でき、共有メモリの実データを表示できるか」だけを
//    検証する。クリックでの切替・位置追従・各プロセスの個別タブバー無効化は
//    まだ実装しない。詳細はchangelog/NKMM_UNIFIED_TABBAR.md参照
//    20260815 Step 1: タブクリックでの実切替(ActivateFrameWindowの呼び出し)、
//    フォアグラウンドのエディタウィンドウへの位置追従(150msポーリング)、
//    各プロセスの個別タブバー(CTabWnd)を常に無効化するよう拡張。
//    タブの並び替え・グループ分離・アイコン表示・複数モニタでのDPI追従は
//    引き続き未実装。
//------------------------------------------------------------------
#define NKMM_UNIFIED_TABBAR
