...
// 修正者
#define SI_AMENDER "silica33"
#define SI_URL     "https://github.com/silica33/sakura"

// 拡張用レジストリキー
#define SI_REGKEY _T("Software\\sakura-silica33")

//------------------------------------------------------------------
// バージョン情報ダイアログの変更 2017.3.15
//------------------------------------------------------------------
#define SI_FIX_VERDLG
...
//------------------------------------------------------------------
#define SI_FIX_CENTERING_CURSOR_JUMP

//------------------------------------------------------------------
// タブ入力文字の切り替え(タブ<->空白)を追加
//  - S_ChangeTabWidthマクロ修正, 負の値を渡すとタブ文字の切り替え
//------------------------------------------------------------------
#define SI_FIX_CHANGE_TAB_WIDTH_MACRO
...
//  - .rcファイルは数が多いので置換対応
//------------------------------------------------------------------
#define SI_FIX_UI_FONT

//------------------------------------------------------------------
// 'BOM付' を 'BOM' に変更 2017.4.17
//  - わざわざ「付」をつけなくても認識できると判断とスペースの節約
//------------------------------------------------------------------
#define SI_FIX_WITH_BOM_TEXT

//------------------------------------------------------------------
// タブ名カラー
//------------------------------------------------------------------
#define SI_FIX_TAB_CAPTION_COLOR
  // 変更ドキュメントタブ名カラー
  // (REG/TabCaptionModifiedColor:#0000d7)
  #define SI_MODIFIED_TAB_CAPTION_COLOR _T("#0000d7")
  // マクロ記録中ドキュメントタブ名カラー
  // (REG/TabCaptionRecMacroColor:#d80000)
  #define SI_RECMACRO_TAB_CAPTION_COLOR _T("#d80000")
...
//  - ブックマークを表示 2017.6.9 
//  - カーソル行を表示 2017.6.11 
//  ? 結果を上に描画しているため現在の範囲がわかりにくい → スクロールバーのカラーとブレンドする
//  ? 更新がいまいち, バーにカーソルを乗せた時 → フェードアウトして消えてしまう;
//------------------------------------------------------------------
#define SI_FIX_EDITVIEW_SCRBAR
  #define SI_SCRBAR_FOUND_MAGIC (0x10000000)  // 検索
  #define SI_SCRBAR_MARK_MAGIC  (0x20000000)  // ブックマーク
  #define SI_SCRBAR_LINEN_MASK  (0x0fffffff)  // 行番号マスク
  #define SI_SCRBAR_MAGIC_MASK  (0xf0000000)  // マジックマスク
  // 検索文字列のある行の色
  // (REG/EditViewScrBarFoundColor:#0000d7)
  #define SI_SCRBAR_FOUND_COLOR  _T("#0000d7")
  // ブックマークのある行の色
  // (REG/EditViewScrBarMarkColor:#d80000)
  #define SI_SCRBAR_MARK_COLOR   _T("#d80000")
  // キャレットのある行の色
  // (REG/EditViewScrBarMarkColor:#d80000)
  #define SI_SCRBAR_CURSOR_COLOR _T("#00d800")
...
//  x キャレットを行間含む高さにする
//------------------------------------------------------------------
#define SI_LINE_CENTERING

//------------------------------------------------------------------
// EOFのみの行にも行番号を表示 2017.3.10
//------------------------------------------------------------------
#define SI_FIX_EOFLN_DISP_NR
...
//------------------------------------------------------------------
#define SI_FIX_TAB_MARK

//------------------------------------------------------------------
// カーソル行アンダーライン
//  - 左端から 2015.1.30
//------------------------------------------------------------------
#define SI_FIX_CUR_UL
...
//  - カラー設定は背景カラーのみ
//------------------------------------------------------------------
#define SI_FIX_SELAREA
  // 選択領域のブレンド率[%] 2015.6.5
  // (REG/SelectAreaTextBlendPer:0x00000000)
  // (REG/SelectAreaBackBlendPer:0x00000064)
  // (REG/SelectAreaBackBlendPer2:0x0000003C)
  #define SI_SELAREA_TEXT_BLEND_PER (0)
  #define SI_SELAREA_BACK_BLEND_PER (100)  // 選択範囲のブレンド率
  #define SI_SELAREA_BACK_BLEND_PER2 (60)  // 特定の下地のときの選択範囲のブレンド率
...
//  - 折り返し記号 テキストカラーのみ
//------------------------------------------------------------------
#define SI_FIX_COLOR_STRATEGY

//------------------------------------------------------------------
// コメント行 2016.12.27
// 0x01: 改行以降もカラーを有効にする (Imitate 'Sublime Text')
//------------------------------------------------------------------
#define SI_FIX_COMMENT

//------------------------------------------------------------------
// 数値の色付け判定
//  - 正規表現で判定する 2017.4.21
//------------------------------------------------------------------
#define SI_FIX_NUMERIC_COLOR
...
//------------------------------------------------------------------
#define SI_FIX_STATUSBAR

//------------------------------------------------------------------
// 折り返しモード 2017.6.3
//  - トグルで切り替えたときに「折り返さない」が処理されていないので修正
//------------------------------------------------------------------
#define SI_FIX_WRAP_MODE
...
//------------------------------------------------------------------
#define SI_FIX_PROFILES
  #define SI_DELETE_HISTORY_NOT_EXIST_AT_STARTUP (1)

//------------------------------------------------------------------
// メインメニューはデフォルトを使用する 2017.5.15
//  - 共通設定から「メインメニュー」タブを削除します
//------------------------------------------------------------------
#define SI_FIX_MAINMENU_FORCE_DEFAULT

//------------------------------------------------------------------
// 開かれているファイルを自己管理する前提で多重オープンの許可 2013.6.19
//  - Shiftを押しながらファイルドロップで多重オープン
//------------------------------------------------------------------
#define SI_MULTIPLE_OPEN_FILES
...
//------------------------------------------------------------------
#define SI_FIX_MAXDATA
  #define SI_MAX_SEARCHKEY  (16) // 検索キー
  #define SI_MAX_REPLACEKEY (16) // 置換キー
  #define SI_MAX_GREPFILE   (8)  // Grepファイル
  #define SI_MAX_GREPFOLDER (16) // Grepフォルダ
...
//------------------------------------------------------------------
#define SI_FIX_RECENT_FILE_DISP_NAME
...
//------------------------------------------------------------------
#define SI_FIX_SEARCH_KEY_REGEXP_AUTO_QUOTE

//------------------------------------------------------------------
// 正規表現検索のときに「単語単位で探す」を使用できるようにする 2017.4.27
//  ! チェック時に検索文字列を \b で囲む
//------------------------------------------------------------------
//#define SI_FIX_ENABLE_WORD_SEARCH_REGEXP
...
//------------------------------------------------------------------
#define SI_FIX_DIALOG_POS
  #define SI_DIALOG_PLACE_TOP  (21)
  #define SI_DIALOG_PLACE_LEFT (21)

//------------------------------------------------------------------
// タグジャンプ一覧
//  - 表示するカラムの選別と並び替え 2015.8.27
//------------------------------------------------------------------
#define SI_FIX_TAGJUMP

//------------------------------------------------------------------
// タグファイル作成ダイアログ
//  - タグ作成フォルダの初期値を tags ファイルがあるところまで辿る 2017.5.12
//------------------------------------------------------------------
#define SI_FIX_TAGMAKEDLG
...
//------------------------------------------------------------------
#define SI_FIX_OUTLINEDLG

//------------------------------------------------------------------
// アウトライン解析
//  - 正規表現を使用した場合はマッチした文字列のみリストに登録 2017.6.8
//------------------------------------------------------------------
#define SI_FIX_OUTLINE
...
//------------------------------------------------------------------
#define SI_FIX_SEARCH_KEEP_REGEXP

//------------------------------------------------------------------
// 行番号が非表示でブックマークが表示のときブックマークは線で描画する 2017.1.13
//------------------------------------------------------------------
#define SI_FIX_DRAW_BOOKMARK_LINE_NOGYOU

//------------------------------------------------------------------
// 行番号縦線を行番号の色で描画する 2016.12.21
//------------------------------------------------------------------
#define SI_FIX_LINE_TERM_TYPE

//------------------------------------------------------------------
// 指定桁縦線の描画桁に違和感…
//  - 処理は正しいが違和感があるので指定した桁の右側に線が引かれるようにする 2017.5.31
//------------------------------------------------------------------
#define SI_FIX_COLUMN_VERTICAL_LINE
