...
//------------------------------------------------------------------
// コードフォールディング(関数/構造体単位の折りたたみ) 20260911
//  - 開発中の段階的実装。フェーズごとにビルド・GUI確認しながら進める
//    Phase1: データモデル(CLineFolded/CFoldManager)+アウトライン解析結果
//      (CFuncInfoArr)から折りたたみ範囲(開始行〜終了行)を算出する
//      CFoldRangeCalculatorのみ追加。表示・カーソル移動等への反映はまだ無い
//    Phase2: CLayoutMgrで折りたたみ範囲内の行をレイアウトから除外(非表示化)
//    Phase3: 行番号マージンに+/-アイコンを描画し、クリックで開閉
//    Phase4: カーソル移動/検索/スクロールの折りたたみ対応
//  - 対象は関数/構造体等のブロック単位のみ(任意の{}やインデントブロックは対象外)
