...
public:
	...
	EConvertResult _UnicodeToHex(const wchar_t* cSrc, const int iSLen, TCHAR* pDst, size_t nDstCount, const CommonSetting_Statusbar* psStatusbar, const bool CESU8Mode);			//!< UNICODE → Hex 変換
	EConvertResult UnicodeToHex(const wchar_t* ps, const int nsl, TCHAR* pd, size_t nDstCount, const CommonSetting_Statusbar* psStatusbar){ return _UnicodeToHex(ps, nsl, pd, nDstCount, psStatusbar, false); }
