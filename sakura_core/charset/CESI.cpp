			 ...
			 && 0 == memcmp( pBuf + i + 1, "encoding", 8 ) ){
				...
				for( k = 0; encodingNameToCode[k].name != NULL; k++ ){
					...
					if( i + nLen < nSize - 1
					  ...
					  && 0 == amemicmp( encodingNameToCode[k].name, pBuf + i, nLen ) ){
...
ECodeType CESI::AutoDetectByHTML( const char* pBuf, int nSize )
{
	for( int i = 0; i + 14 < nSize; i++ ){
		...
		// 「<meta charset="utf-8">」
		if( pBuf[i] == '<' && 0 == amemicmp(&pBuf[i+1], "meta", 4) && IsXMLWhiteSpace(pBuf[i+5]) ){
			i += 5;
			...
			while( i < nSize ){
				if( IsXMLWhiteSpace(pBuf[i]) && i + 1 < nSize && !IsXMLWhiteSpace(pBuf[i+1]) ){
					int nAttType = 0;
					i++;
					if( i + 12 < nSize && 0 == amemicmp(pBuf + i, "http-equiv", 10)
						&& (IsXMLWhiteSpace(pBuf[i+10]) || '=' == pBuf[i+10]) ){
						i += 10;
						nAttType = 1; // http-equiv
					}else if( i + 9 < nSize && 0 == amemicmp(pBuf + i, "content", 7)
						&& (IsXMLWhiteSpace(pBuf[i+7]) || '=' == pBuf[i+7]) ){
						i += 7;
						nAttType = 2; // content
					}else if( i + 9 < nSize && 0 == amemicmp(pBuf + i, "charset", 7)
						&& (IsXMLWhiteSpace(pBuf[i+7]) || '=' == pBuf[i+7]) ){
						i += 7;
						nAttType = 3; // charset
					}else{
					...
					if( 1 == nAttType ){
						// http-equiv
						if( 12 == nEndAttVal - nBeginAttVal && 0 == amemicmp(pBuf + nBeginAttVal, "content-type", 12) ){
							bContentType = true;
							if( encoding != CODE_NONE ){
								return encoding;
							}
						...
						if( nEndAttVal <= i ){ i = nNextPos; continue; }
						if( i + 7 < nEndAttVal && 0 == amemicmp(pBuf + i, "charset", 7) ){
							i += 7;
							while( i < nEndAttVal && IsXMLWhiteSpace(pBuf[i]) ){ i++; }
							if( nEndAttVal <= i ){ i = nNextPos; continue; }
							...
							for( k = 0; encodingNameToCode[k].name != NULL; k++ ){
								...
								if( i - nCharsetBegin == nLen
								  && 0 == amemicmp( encodingNameToCode[k].name, pBuf + nCharsetBegin, nLen ) ){
									if( bContentType ){
										return static_cast<ECodeType>(encodingNameToCode[k].nCode);
									}else{
						...
						for( k = 0; encodingNameToCode[k].name != NULL; k++ ){
							...
							if( nEndAttVal - nBeginAttVal == nLen
							  && 0 == amemicmp( encodingNameToCode[k].name, pBuf + nBeginAttVal, nLen ) ){
								return static_cast<ECodeType>(encodingNameToCode[k].nCode);
							}
						}
					}
	...
	for( int i = 0; i + 8 < nSize; i++ ){
		if( bComment && 0 == memcmp(pBuf + i, "coding", 6)
			...
			for( k = 0; encodingNameToCode[k].name != NULL; k++ ){
				const int nLen = encodingNameToCode[k].nLen;
				if( i - nBegin == nLen
				  && 0 == amemicmp( encodingNameToCode[k].name, pBuf + nBegin, nLen ) ){
					return static_cast<ECodeType>(encodingNameToCode[k].nCode);
				}
			}
		}else if( '\r' == pBuf[i] || '\n' == pBuf[i] ){
