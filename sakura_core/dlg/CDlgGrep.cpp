...
//2002/03/07実装してみた。
	...
	case IDC_BUTTON_FOLDER2:
		/* フォルダ参照ボタン */
		{
			...
			TCHAR	szFolder[nMaxPath];
			/* 検索フォルダ */
			::DlgItem_GetText( GetHwnd(), IDC_COMBO_FOLDER2, szFolder, nMaxPath - 1 );
			if( szFolder[0] == _T('\0') ){
				::GetCurrentDirectory( nMaxPath, szFolder );
			}
			if( SelectDir( GetHwnd(), LS(STR_DLGGREP1), szFolder, szFolder ) ){
	...
	case IDC_BUTTON_FOLDER3:
		/* フォルダ参照ボタン */
		{
			...
			TCHAR	szFolder[nMaxPath];
			/* 検索フォルダ */
			::DlgItem_GetText( GetHwnd(), IDC_COMBO_FOLDER3, szFolder, nMaxPath - 1 );
			if( szFolder[0] == _T('\0') ){
				::GetCurrentDirectory( nMaxPath, szFolder );
			}
			if( SelectDir( GetHwnd(), LS(STR_DLGGREP1), szFolder, szFolder ) ){
...
//	}
...

	{
		...
		auto_strcpy_s( m_szFolder, _countof2(m_szFolder), szFolder );
	}
