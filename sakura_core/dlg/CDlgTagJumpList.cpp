...
void CDlgTagJumpList::UpdateData( bool bInit )
{
	...
	for( nIndex = 0; nIndex < count; nIndex++ )
	{
		...
		ListView_SetItemText( hwndList, nIndex, 2, p );
		free( p );
		
		ListView_SetItemText( hwndList, nIndex, 3, item->note );
#else
