...
namespace {
	...
	*/
	...
	const MultiMoraKanjiEntry	g_aMultiMoraKanjiTable[] = {
		...
		{ L'違', L"ちが" },	// 違う・違います
		{ L'使', L"つか" },	// 使う・使えません
		{ L'上', L"うえ" },	// 上移動・カーソルの上移動等(1漢字1読みの制約上、「上書き」の
							// 連濁「うわ」とは択一。パレット中の出現数は「上移動」系の方が
							// 多いためこちらを優先した。「上書き保存」を"uwagaki"で探す経路は
							// 退行するが、"jouSHOki"/"hozon"等の他の入力や、既存のショートカット
							// (Ctrl+S)での利用は妨げない 20260822
		{ L'直', L"なお" },	// 直す
		{ L'検', L"けん" },	// 検索
		{ L'索', L"さく" },	// 検索
		...
		{ L'端', L"はし" },	// 左端・右端等(訓読み。旧登録の音読み「たん」は誤りだった) 20260822
		...
		{ L'付', L"つ" },	// 貼り付け・引用符付き・名前を付けて等(訓読み語幹。単独モーラ
							...
							// よる新規の後退ではない)
		{ L'番', L"ばん" },	// 番号(単独モーラ辞書は清音化した「は」の行。2モーラ目欠けも対策) 20260822
		{ L'号', L"ごう" },	// 番号(単独モーラ辞書は清音化した「こ」の行。2モーラ目欠けも対策) 20260822
		{ L'引', L"いん" },	// 引用符付きコピー(2モーラ目欠け対策) 20260822
		{ L'用', L"よう" },	// 引用符付きコピー(2モーラ目欠け対策) 20260822
	};
