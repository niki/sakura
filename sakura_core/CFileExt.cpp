...
bool CFileExt::AppendExt( const TCHAR *pszName, const TCHAR *pszExt )
{
	...
	if( !CDocTypeManager::ConvertTypesExtToDlgExt( pszExt, NULL, szWork, _countof(szWork) ) ) return false;
...
bool CFileExt::AppendExtRaw( const TCHAR *pszName, const TCHAR *pszExt )
{
	...
	auto_strcpy( m_puFileExtInfo[m_nCount].m_szName, pszName );
	auto_strcpy( m_puFileExtInfo[m_nCount].m_szExt, pszExt );
	m_nCount++;
