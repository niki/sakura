...
{
	//名前と拡張子
	auto_strcpy( pType->m_szTypeName, _T("COBOL") );
	auto_strcpy( pType->m_szTypeExts, _T("cbl,cpy,pco,cob") );	//Jun. 04, 2001 JEPRO KENCH氏の助言に従い追加
...
//			MYTRACE( _T("szLabel=[%ls]\n"), szLabel );
			...
			for( i = 0; i <= nLen ; ++i ){
				if( 0 == auto_memicmp( &szLabel[i], pszKeyWord, nKeyWordLen ) ){
					...
					auto_strcpy( szDivision, szLabel );
					bDivision = TRUE;
					break;
				}
			}
...
#include "generated/cobol_keywords.inc"
};
