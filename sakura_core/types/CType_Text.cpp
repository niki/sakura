...
{
	//名前と拡張子
	auto_strcpy( pType->m_szTypeName, _T("テキスト") );
	auto_strcpy( pType->m_szTypeExts, _T("txt,log,1st,err,ps") );
	...
	auto_strcpy( pType->m_szKinsokuHead, L"!%),.:;?]}\xa2°’”‰′″℃、。々〉》」』】〕゛゜ゝゞ・ヽヾ！％），．：；？］｝｡｣､･ﾞﾟ￠" );		/* 行頭禁則 */	//@@@ 2002.04.13 MIK 
	auto_strcpy( pType->m_szKinsokuTail, L"$([\\{\xa3\xa5‘“〈《「『【〔＄（［｛｢￡￥" );		/* 行末禁則 */	//@@@ 2002.04.08 MIK 
...
void CDocOutline::MakeTopicList_txt( CFuncInfoArr* pcFuncInfoArr )
{
	...
	*/
	...
	for( nLineCount = CLogicInt(0); nLineCount <  m_pcDocRef->m_cDocLineMgr.GetLineCount(); ++nLineCount )
	{
		...
		//見出し種類の判別 -> szTitle
		if( pLine[i] == L'(' ){
			     if ( IsInRange(pLine[i + 1], L'0', L'9') ) auto_strcpy( szTitle, L"(0)" ); //数字
			else if ( IsInRange(pLine[i + 1], L'A', L'Z') ) auto_strcpy( szTitle, L"(A)" ); //英大文字
			else if ( IsInRange(pLine[i + 1], L'a', L'z') ) auto_strcpy( szTitle, L"(a)" ); //英小文字
			else continue; //※「(」の次が英数字で無い場合、見出しとみなさない
		}
		else if( IsInRange(pLine[i], L'０', L'９') ) auto_strcpy( szTitle, L"０" ); // 全角数字
		else if( IsInRange(pLine[i], L'①', L'⑳') || pLine[i] == L'\u24ea'
			|| IsInRange(pLine[i], L'\u3251', L'\u325f') || IsInRange(pLine[i], L'\u32b1', L'\u32bf') ) auto_strcpy( szTitle, L"①" ); // ①～⑳ ○0　○21○35　○36○50
		else if( IsInRange(pLine[i], L'Ⅰ', L'\u216f') ) auto_strcpy( szTitle, L"Ⅰ" ); // Ⅰ～Ⅹ　XIXIILCDM
		else if( IsInRange(pLine[i], L'ⅰ', L'\u217f') ) auto_strcpy( szTitle, L"Ⅰ" ); // Ⅰ～Ⅹ　xixiilcdm
		else if( IsInRange(pLine[i], L'\u2474', L'\u2487') ) auto_strcpy( szTitle, L"\u2474" ); // (1)-(20)
		else if( IsInRange(pLine[i], L'\u2488', L'\u249b') ) auto_strcpy( szTitle, L"\u2488" ); // 1.-20.
		else if( IsInRange(pLine[i], L'\u249c', L'\u24b5') ) auto_strcpy( szTitle, L"\u249c" ); // (a)-(z)
		else if( IsInRange(pLine[i], L'\u24b6', L'\u24cf') ) auto_strcpy( szTitle, L"\u24b6" ); // ○A-○Z
		else if( IsInRange(pLine[i], L'\u24d0', L'\u24e9') ) auto_strcpy( szTitle, L"\u24d0" ); // ○a-○z
		else if( IsInRange(pLine[i], L'\u24eb', L'\u24f4') ){ // ●11-●20
			if(b278a){ auto_strcpy( szTitle, L"\u278a" ); }
			else{ auto_strcpy( szTitle, L"\u2776" ); } }
		else if( IsInRange(pLine[i], L'\u24f5', L'\u24fe') ) auto_strcpy( szTitle, L"\u24f5" ); // ◎1-◎10
		else if( IsInRange(pLine[i], L'\u2776', L'\u277f') ) auto_strcpy( szTitle, L"\u2776" ); // ●1-●10
		else if( IsInRange(pLine[i], L'\u2780', L'\u2789') ) auto_strcpy( szTitle, L"\u2780" ); // ○1-○10
		else if( IsInRange(pLine[i], L'\u278a', L'\u2793') ){ auto_strcpy( szTitle, L"\u278a" ); b278a = true; } // ●1-●10(SANS-SERIF)
		else if( IsInRange(pLine[i], L'\u3220', L'\u3229') ) auto_strcpy( szTitle, L"\ua3220" ); // (一)-(十)
		else if( IsInRange(pLine[i], L'\u3280', L'\u3289') ) auto_strcpy( szTitle, L"\u3220" ); // ○一-○十
		else if( IsInRange(pLine[i], L'\u32d0', L'\u32fe') ) auto_strcpy( szTitle, L"\u32d0" ); // ○ア-○ヲ
		else if( wcschr(L"〇一二三四五六七八九十百零壱弐参伍", pLine[i]) ) auto_strcpy( szTitle, L"一" ); //漢数字
		else{
