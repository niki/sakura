...
**Changed**  
+ ファイル系
  - 履歴を別ファイル (`sakura.recent.json`)に出力  
  - ~~起動時に存在しない履歴を削除する~~  
  - デフォルト設定ファイル (`sakura.default.ini`)の使用  
  - 強調キーワード設定は `sakura.keywordset.json`から読み込む  
...
+ 表示系
  ...
  - 行を中央ぞろえにする (デフォルトでは上揃えになっていて行の間隔が下に付加されている)  
...
+ UI系
  ...
  - 選択タブのアクティブ化をマウス押下時に行う (デフォルトではマウス押上時にアクティブにするのでワンテンポ遅く感じる)  
