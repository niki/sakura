...
public:
	...
	std::tstring &At(int index)
	{
		...
		/// xxx: トークンありき（index は常に有効な範囲で呼び出される前提）
		/// warning C4715 対応	2026.07.31 yuu_zuki
		/// 呼び出し側の契約違反時のフォールバックとして空文字列への参照を返す
		static std::tstring empty;
		return empty;
	}
	...
	void Split(const std::tstring &text, TCHAR delimiter)
	{
		...
		TCHAR *del_temp = temp;
		_tcscpy_s(temp, text.size() + 1, text.c_str());
		temp[text.size()] = 0;
