...
4. **Shift/Ctrl/Alt＋キーのキーセット指定UI**: 指定した組み合わせが既存の
   一覧行と一致すればスクロールしてフォーカスする(逆方向の同期は「追記」
   節を参照)。
...
## 追記: 一覧選択→「キー入力」欄への同期 20260810

対象ファイル: `sakura_core/prop/CPropComKeybindList.cpp`
(`UpdateCaptureText()`新設)、`sakura_core/prop/CPropCommon.h`(宣言追加)。

「機能名/キーの一覧」から行を選ぶ(`ReflectSelection()`)操作は、上部の
コンボボックス＋Shift/Ctrl/Altチェックボックスには反映されるが、直接入力用の
「キー入力」欄(`IDC_EDIT_KEYBINDLIST_CAPTURE`)だけは更新されず、直前に
手動でキーを叩いたときの表示が残ったままになっていた。

`WM_NKMM_KEYBINDLIST_CAPTURE`ハンドラ(実際にキーを押したときの表示更新)
にあった文字列組み立てロジックを`UpdateCaptureText( HWND, int nKeyIndex )`
として切り出し、`ReflectSelection()`からも呼ぶようにした。これにより
一覧の行をダブルクリック(またはクリックで選択)したときも、直接入力欄に
選んだキーの組み合わせ("Ctrl+Z"等)が即座に反映されるようになった。
`nKeyIndex`が負の場合は欄を空にしてプレースホルダー("キーを押す")表示に戻す。
