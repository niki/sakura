...
| v2.4.3.key | サクラエディタ本家 v2.4.3（他プリセットと違い他アプリではなく本家の既定キーへ戻すためのもの） |
...
- **TeraPad.key は追加していません。** Web検索で確認した限り、断片的なブログ記事はあるものの
  ...
  手元にあれば作成できます。
- **v2.4.3.key を追加しました(20260905)。** 他のプリセットが「別のエディタ/IDEの操作感を
  サクラに持ち込む」ものであるのに対し、これだけは目的が逆で「本フォークが独自に変更した
  既定キー割り当てを、サクラエディタ本家(upstream/release-2.4.3)の既定へ戻す」ためのものです。
  ローカルの`upstream/release-2.4.3`ブランチから`sakura_core/func/CKeyBind.cpp`の
  `KeyDataInit[]`(本家には`#ifdef NKMM_*`が一切無い素の状態)を取得し、機能コードの数値
  (`sakura_core/Funccode_x.hsrc`)が本フォークと完全に一致することを確認した上で、機能名
  ベースで本フォークの現行既定値(`docs/sakura_keybind_list.html`の「サクラ既定」列)と
  物理キー・モディファイア単位で突き合わせています。実際に割り当てが食い違う20物理キー・
  32スロットのみを収録(例: `Ctrl+D`は本フォークでは「次の一致を選択範囲に追加」だが本家では
  「単語切り取り」、`Ctrl+H`は本フォークでは置換ダイアログだが本家では「カーソル行を中央へ」)。
  なお、ダブル/トリプル/クアドラプルクリックやマウスホイールの疑似キー(`VKEX_*`)は、本家では
  全モディファイアスロットに同一機能を単純repeatしているだけ(例: ダブルクリックはShift/Ctrl/Alt
  を押していても常に単語選択)で、無変更のスロット0以外は実質的な挙動差では無いため対象外と
  しました。マウス系機能キー(`VK_APPS`)も同様の理由で対象外です。
...
**`Default.key`は20260825に、`sakura_core/func/CKeyBind.cpp`の`KeyDataInit[]`から
機械的に再生成したものに差し替えました。** 従来のファイルは一部の機能コードが数値ではなく
`SelectWord`/`WheelUp`のような旧来のシンボル名で書かれており、他の全プリセットと形式が
揃っていませんでした。再生成後は他プリセットと同じ数値コード形式に統一し、`KeyDataInit[]`の
全100件(マウスクリック/ホイールの疑似キー`VKEX_*`9件、割り当てが1つも無い空行15件を含む)を
そのまま反映しています。キー名の後ろのTAB区切り注釈は付けていません(旧ファイルとは異なりますが、
インポート結果には影響しません)。なお、実アプリの共通設定「キー割り当て」プリセットコンボの
先頭にある「デフォルト」項目はこのファイルを使わず、ビルド時点の`KeyDataInit[]`を
`CShareData::ResetKeyBindToDefault()`で直接反映する設計になっているため
(`sakura_core/prop/CPropComKeybindList.cpp`の`s_KeybindPresetInfo`コメント参照)、
`Default.key`が今後また実際の既定値とズレても実アプリの動作には影響しません。あくまで
`docs/sakura_keybind_list.html`の生成時など、既定値のスナップショットが欲しい用途向けの
参考ファイルです。
